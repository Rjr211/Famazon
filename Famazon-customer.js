...
//connect to the mysql database and pull the information from the Products database to display to user

connection.query('SELECT ItemID, ProductName, Price FROM Products', function(err, result){
	if(err) console.log(err);

	//create table for the information from the mysql database to be placed
	var table = new Table({
...
var purchase = function(){
	...
	var productInfo = {
		properties: {
			itemID:{description: colors.blue('Please enter the ID # of the item you want to purchase')},
	...
	prompt.get(productInfo, function(err, res){
		...
		//the variable established is pushed to the productPurchased array defined at the top of the page
